...
export class AppComponent implements OnInit, OnDestroy {
  ...
  initCharacterEvents(): void {
    ...
    this.charactersResults$ = this.searchTermByCharacters
        // YOUR CODE STARTS HERE
        .pipe(filter(inputValue => inputValue.length >= 3))
        .pipe(debounceTime(300))
        .pipe(switchMap(x => this.mockDataService.getCharacters(x)));
        // YOUR CODE ENDS HERE
  }
  ...
  initLoadingState(): void {
    ...
    // YOUR CODE STARTS HERE
      const combinedLoaderState = combineLatest([this.mockDataService.getCharactersLoader(), this.mockDataService.getPlanetLoader()]);
      this.subscriptions.push(combinedLoaderState.subscribe(value => { 
